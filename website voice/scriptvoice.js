const medicalProblems = [
  ...
  document.getElementById('queryForm').addEventListener('submit', function(e) {
    ...
    for (let i = 0; i < medicalProblems.length; i++) {
      if (medicalProblems[i].name.toLowerCase() === userQuery) {
        ...
        responseDiv.innerHTML = `<h2>${medicalProblems[i].name}</h2><p>${medicalProblems[i].description}</p><br><h1>Treatment:</h1><p>${medicalProblems[i].treatment}</p>`;
        found = true;
        break;
      }
    }
  ...
  recognition.onresult = (event) => {
    ...
    var userQuery=userVoiceInp.toLowerCase();
    outputDiv.innerHTML = '';
    ...
    for (let i = 0; i < medicalProblems.length; i++) {
      if (medicalProblems[i].name.toLowerCase() === userQuery) {
        responseText = `${medicalProblems[i].name}: ${medicalProblems[i].description}:$Treatment: ${medicalProblems[i].treatment}`;
        outputDiv.innerHTML = `<h2>${medicalProblems[i].name}:</h2><p>${medicalProblems[i].description}</p><h2>Treatment:</h2><p>${medicalProblems[i].treatment}</p>`;
        found = true;
        break;
      }
    }
